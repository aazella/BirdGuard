...
# ================== SHARED STATE ================
latest_frame = None
detected_now = 0            # 1 if current frame has ≥ conf illegal
lock = threading.Lock()

CONF_TRIGGER = 0.10
REQUIRED_ON_TIME = 1        # illegal must be detected 1 second continuously
...
def yolo_thread():
    ...
    while True:
        ...
        for det in results[0].boxes:
            ...
            if cls == 1 and conf >= CONF_TRIGGER:
                detected_now = 1
                break
...
print("🚀 Running: LED ON only if illegal is detected for 1 full second")
...
while True:
    ...
    if detected_now:
        ...
        # detection lasted 1 full second → turn ON
        if now - detection_start_time >= REQUIRED_ON_TIME:
            lgpio.gpio_write(chip, LED_PIN, 1)

    else:
        # detection interrupted → reset timer & turn OFF
        detection_start_time = None
        lgpio.gpio_write(chip, LED_PIN, 0)
